...
export default function Campaigns({ campaigns, loading, error, setExpandedCampaign }) {
  ...
  return (
    <div className="campaigns-list">
      {campaigns.map((campaign, idx) => (
        <CampaignCard key={campaign.id || idx} campaign={campaign} onClick={() => setExpandedCampaign && setExpandedCampaign(campaign)} />
      ))}
    </div>
  );
}
